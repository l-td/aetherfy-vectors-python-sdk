...
def timed_client(api_key, test_endpoint, mock_requests):
    ...
    return AetherfyVectorsClient(
        api_key=api_key, endpoint=test_endpoint, timeout=30.0
    )
...
class TestComputeBodyAwareTimeout:
    ...
    def test_custom_base_timeout_scales_from_that_base(self, api_key, test_endpoint, mock_requests):
...
class TestTimeoutWiringIntoRequests:
    ...
    def test_get_uses_base_timeout(self, client, mock_requests, mock_successful_response):
    ...
    def test_small_post_uses_base_timeout(self, client, mock_requests, mock_successful_response):
        ...
        client.create_collection("c", VectorConfig(size=4, distance=DistanceMetric.COSINE))
        assert self._last_request_timeout(mock_requests) == client.timeout

    def test_large_upsert_put_scales_timeout(self, client, mock_requests, mock_successful_response):
        ...
        assert put_timeout > client.timeout, (
