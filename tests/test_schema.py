...
class TestFieldDefinition:
    ...
    def test_field_definition_with_nested_fields(self):
        ...
        nested_fields = {"source": FieldDefinition(type="string", required=True)}
    ...
    def test_field_definition_to_dict_with_element_type(self):
        ...
        result = field.to_dict()
        assert result == {"type": "array", "required": True, "element_type": "integer"}
        ...
        schema = Schema(
            fields={
                "metadata": FieldDefinition(
                    type="object",
                    required=True,
                    fields={"source": FieldDefinition(type="string", required=True)},
                )
            }
        )
        result = schema.to_dict()
...
class TestValidatePayload:
    ...
    def test_validate_missing_required_field(self):
        ...
        schema = Schema(fields={"name": FieldDefinition(type="string", required=True)})
        payload = {}
        errors = validate_payload(payload, schema)
        assert len(errors) == 1
    ...
    def test_validate_type_mismatch(self):
        ...
        schema = Schema(
            fields={"price": FieldDefinition(type="integer", required=True)}
        )
        payload = {"price": "100"}
        errors = validate_payload(payload, schema)
        assert len(errors) == 1
        ...
        schema = Schema(
            fields={
                "tags": FieldDefinition(
                    type="array", required=True, element_type="string"
                )
            }
        )
        payload = {"tags": ["a", "b", "c"]}
...
class TestAnalysisResult:
    ...
    def test_analysis_result_from_dict(self):
        ...
        data = {
            ...
            "total_points": 1000,
            "fields": {
                "name": {"presence": 1.0, "types": {"string": 1.0}, "warnings": []},
                "price": {
                    "presence": 0.9,
...
class TestClientSchemaOperations:
    ...
    def test_get_schema_not_found(self, client, mock_requests, mock_error_response):
        ...
        mock_requests.request.return_value = mock_error_response(
            message="Schema not found", status_code=404, error_code="SCHEMA_NOT_FOUND"
        )
    ...
    def test_set_schema_success(self, client, mock_requests, mock_successful_response):
        ...
        mock_requests.request.return_value = mock_successful_response(
            {"success": True, "etag": "new_etag_123"}, 200
        )
    ...
    def test_analyze_schema_success(
        self, client, mock_requests, mock_successful_response
    ):
        ...
        analysis_data = {
            ...
            "fields": {
                "name": {"presence": 1.0, "types": {"string": 1.0}, "warnings": []},
                "price": {"presence": 0.95, "types": {"integer": 1.0}, "warnings": []},
            },
            "suggested_schema": {
                "fields": {
                    "name": {"type": "string", "required": True},
    ...
    def test_analyze_schema_with_default_sample_size(
        self, client, mock_requests, mock_successful_response
    ):
    ...
    def test_set_schema_404_evicts_caches(
    ...
    def test_analyze_schema_404_evicts_caches(
        self, client, mock_requests, mock_error_response
    ):
    ...
    def test_refresh_schema(self, client, mock_requests, mock_successful_response):
        ...
        client._payload_schema_cache["test_collection"] = {
            "schema": Schema(
                fields={"old": FieldDefinition(type="string", required=True)}
            ),
            "enforcement": "strict",
            "etag": "old_etag",
        }
        ...
        new_schema_data = {
            "schema": {"fields": {"new": {"type": "integer", "required": True}}},
...
class TestClientValidationIntegration:
    ...
    def test_upsert_with_valid_data_and_strict_schema(
        self, client, mock_requests, mock_successful_response
    ):
    ...
    def test_upsert_with_invalid_data_raises_validation_error(
        self, client, mock_requests, mock_successful_response
    ):
        ...
        points = [
            Point(
                id=1,
                vector=[0.1, 0.2],
                payload={"name": "Product A", "price": "invalid"},
            )
        ]
    ...
    def test_upsert_without_schema_allows_any_data(
        self, client, mock_requests, mock_error_response, mock_successful_response
    ):
        ...
        points = [Point(id=1, vector=[0.1, 0.2], payload={"anything": "goes"})]

        result = client.upsert("test_collection", points)
        assert result is True

    def test_upsert_with_warn_mode_allows_invalid_data(
        self, client, mock_requests, mock_successful_response
    ):
        ...
        points = [Point(id=1, vector=[0.1, 0.2], payload={"name": 123})]  # Wrong type

        result = client.upsert("test_collection", points)
        assert result is True

    def test_upsert_handles_412_etag_mismatch(
        self, client, mock_requests, mock_successful_response, mock_error_response
    ):
        ...
        points = [Point(id=1, vector=[0.1, 0.2], payload={"name": "Product A"})]
    ...
    def test_upsert_with_off_enforcement_skips_validation(
        self, client, mock_requests, mock_successful_response
    ):
