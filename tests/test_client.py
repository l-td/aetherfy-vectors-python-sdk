...
class TestClientInitialization:
    ...
    def test_client_init_with_api_key(self, api_key, test_endpoint):
        ...
        client = AetherfyVectorsClient(
            api_key=api_key, endpoint=test_endpoint, timeout=15.0
        )
...
class TestCollectionManagement:
    ...
    def test_create_collection_with_dict_config(
        self, client, mock_requests, mock_successful_response
    ):
    ...
    def test_create_collection_with_description(
        ...
        result = client.create_collection(
            "test_collection", config, description=description
        )
...
class TestPointOperations:
    ...
    def test_upsert_point_objects(
        ...
        points = [
            Point(id=1, vector=[0.1, 0.2, 0.3], payload={"test": True}),
            Point(id=2, vector=[0.4, 0.5, 0.6]),
        ]
    ...
    def test_delete_points_by_ids(
        self, client, mock_requests, mock_successful_response
    ):
        """Test point deletion by IDs."""
        mock_requests.request.return_value = mock_successful_response({})

        result = client.delete("test_collection", [1, 2])

        assert result is True
        args, kwargs = mock_requests.request.call_args
        assert kwargs["method"] == "POST"
        assert "points/delete" in kwargs["url"]
        assert kwargs["json"]["points"] == [1, 2]

    def test_delete_points_by_filter(
        self, client, mock_requests, mock_successful_response
    ):
    ...
    def test_retrieve_points_success(
        self, client, mock_requests, mock_successful_response
    ):
        ...
        points_data = {
            "result": [
                {"id": 1, "vector": [0.1, 0.2, 0.3], "payload": {"category": "test"}}
            ]
        }
        mock_requests.request.return_value = mock_successful_response(points_data)

        points = client.retrieve("test_collection", [1], with_vectors=True)
...
class TestSearchOperations:
    ...
    def test_search_with_filter(
        ...
        results = client.search(
            "test_collection", query_vector, query_filter=query_filter
        )
